...
const InstagramConnect = () => {
  ...
  const [tempDeployType, setTempDeployType] = useState<"auto" | "manual">(
    "auto"
  );
  ...
  const checkTokenExpiration = async () => {
    try {
      ...
      if (
        !data?.data?.cafe24ExpiresAt ||
        new Date(data.data.cafe24ExpiresAt) <= new Date()
      ) {
  ...
  const disconnectInstagram = async () => {
    try {
      ...
      setError(
        "Instagram 연동 해제 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요."
      );
          ...
          <Alert className="bg-gradient-to-r from-[#fdf5e6] to-[#fef1f6] border-[#fbcac9]">
            <AlertDescription className="flex items-center justify-between flex-wrap gap-3">
              <div className="flex items-center gap-2">
                ...
                <span>
                  Instagram 계정을 연동하여 쇼핑몰에 피드를 자동으로 게시할 수
                  있습니다.
                </span>
              </div>
              <Button
              ...
              >
                <a
                  ...
                  className={showTokenAlert ? "pointer-events-none" : ""}
                >
                  Instagram 연동하기
                </a>
              </Button>
            ...
            <div className="space-y-6">
                  ...
                  {tempDeployType !== status.insertType && (
                    <div className="flex gap-2">
                      ...
                      <Button
                        variant="outline"
                        onClick={() =>
                          setTempDeployType(status.insertType || "auto")
                        }
                        disabled={isUpdatingType}
                        className="flex-1"
                      >
              ...
              {status.insertType === "auto" ? (
                <p className="text-sm text-gray-600 bg-gray-50 p-4 rounded-md">
                  자동 배포를 선택하면 쇼핑몰 메인의 footer 요소 상단에 자동으로
                  피드가 표시됩니다.
                  {status.hasScriptTag && (
                ...
                <div className="space-y-4">
                  <p className="text-sm text-gray-600 bg-gray-50 p-4 rounded-md">
                    수동 배포의 경우 스크립트 배포 이후 아래 코드를 원하는
                    위치에 직접 삽입하세요.
                    <strong className="mt-2 text-red-600 block">
                      <CircleAlert className="inline-block" /> 스크립트는
                      제거하지 마세요. 스크립트를 제거하면 피드가 동작하지
                      않습니다.
                    </strong>
                  </p>
                  <CodeBlock
